...
* the video chunks in a `chunks/` folder next to the video file, e.g. `chunks/00000000-00000900.mp4` (this folder persists only when `cache_chunks` is true; otherwise the chunks are removed once the video has been processed)

## Method

The video is split into chunks of `chunk_duration_secs` seconds (15 minutes by default), consecutive chunks overlapping by `chunk_overlap_secs` seconds (2 minutes by default). Chunks are cut with ffmpeg: the seek is fast and the chunk is re-encoded (`libx264`, `veryfast`, crf 28) so that cut points are frame-accurate. When `cache_chunks` is true, existing chunks are reused across runs and kept on disk; when it is false (default), the chunks are removed once the video has been processed.
...
* `cache_chunks`: keep the chunks on disk after processing (default: false) so that they are reused across runs
